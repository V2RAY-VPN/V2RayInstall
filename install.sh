...
path_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认: [${cyan}test$none]):")" path
		[[ -z $path ]] && path="test"
...
proxy_site_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认: [${cyan}https://test.com$none]):")" proxy_site
		[[ -z $proxy_site ]] && proxy_site="https://test.com"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}test.com$none)"): " sspass
		[ -z "$sspass" ] && sspass="test.com"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你完整上传 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		mkdir -p /etc/v2ray/test/v2ray
		cp -rf $(pwd)/* /etc/v2ray/test/v2ray
	else
		if [[ $_test ]]; then
			git clone https://github.com/V2RAY-VPN/V2RayInstall -b test /etc/v2ray/test/v2ray
		else
			git clone https://github.com/V2RAY-VPN/V2RayInstall /etc/v2ray/test/v2ray
		fi

	fi

	if [[ ! -d /etc/v2ray/test/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/test/v2ray/config/backup.conf $backup
...
backup_config() {
	...
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=test.com/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] ]] && sed -i "36s/=test.com/=$domain/" $backup
	[[ $caddy ]] && sed -i "39s/=/=true/" $backup
	[[ $ban_ad ]] && sed -i "54s/=/=true/" $backup
	if [[ $is_path ]]; then
		sed -i "57s/=/=true/; 60s/=test/=$path/" $backup
		sed -i "63s#=https://test.com#=$proxy_site#" $backup
	fi
}
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/test/v2ray ]]; then
		...
		echo -e "
		$red 大胸弟...你貌似没有安装 V2Ray ....卸载个鸡鸡哦...$none

		备注...仅支持卸载 V2Ray 一键安装脚本
		" && exit 1
	fi
