...
if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/test/v2ray ]]; then

	. $backup

elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/v2ray_backup.txt && -d /etc/v2ray/test/v2ray ]]; then

	. /etc/v2ray/test/v2ray/tools/v1xx_to_v3xx.sh
...
if [[ $mark != "v3" ]]; then
	. /etc/v2ray/test/v2ray/tools/v3.sh
fi
...
. /etc/v2ray/test/v2ray/src/init.sh
systemd=true
# _test=true
...
get_shadowsocks_config_qr_link() {
	if [[ $shadowsocks ]]; then
		...
		echo
		get_ip
		local ss="ss://$(echo -n "${ssciphers}:${sspass}@${ip}:${ssport}" | base64 -w 0)#test.com_ss_${ip}"
		echo "${ss}" >/tmp/shadowsocks.txt
		cat /tmp/shadowsocks.txt | qrencode -s 50 -o /tmp/shadowsocks.png

		local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
		local link=$(curl -s --upload-file /tmp/_shadowsocks.png "https://transfer.sh/${random}_shadowsocks.png")
		if [[ $link ]]; then
			echo
		...
		rm -rf /tmp/shadowsocks.png
		rm -rf /tmp/shadowsocks.txt
	else
		shadowsocks_config
	fi
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}test.com$none)"): " new_sspass
		[ -z "$new_sspass" ] && new_sspass="test.com"
		case $new_sspass in
		*[/$]*)
			echo
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta}用来分流的路径$none , 例如 /test , 那么只需要输入 test 即可"
		read -p "$(echo -e "(默认: [${cyan}test$none]):")" new_path
		[[ -z $new_path ]] && new_path="test"
...
proxy_site_config() {
	echo
	while :; do
		echo -e "请输入 ${magenta}一个正确的$none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://test.com"
		echo -e "举例...假设你当前的域名是$green $domain $none, 伪装的网址的是 https://test.com"
		echo -e "然后打开你的域名时候...显示出来的内容就是来自 https://test.com 的内容"
		echo -e "其实就是一个反代...明白就好..."
		echo -e "如果不能伪装成功...可以使用 v2ray config 修改伪装的网址"
		read -p "$(echo -e "(默认: [${cyan}https://test.com$none]):")" new_proxy_site
		[[ -z $new_proxy_site ]] && new_proxy_site="https://test.com"
...
change_proxy_site_config() {
	if [[ $v2ray_transport == [45] ]] && [[ $caddy && $is_path ]]; then
		...
		while :; do
			echo -e "请输入 ${magenta}一个正确的$none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://test.com"
			echo -e "举例...你当前的域名是$green $domain $none, 伪装的网址的是 https://test.com"
			echo -e "然后打开你的域名时候...显示出来的内容就是来自 https://test.com 的内容"
			echo -e "其实就是一个反代...明白就好..."
			echo -e "如果不能伪装成功...可以使用 v2ray config 修改伪装的网址"
	...
	while :; do
		...
		if [[ -z $is_xshell ]]; then
			...
			if [[ $is_xshell == [yY] ]]; then
				...
				echo
				# echo -e "$yellow 解压密码 = ${cyan}test.com$none"
				# echo
...
get_v2ray_config_link() {
	...
	echo
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file $v2ray_client_config "https://transfer.sh/${random}_v2ray.json")
	if [[ $link ]]; then
		echo
...
get_v2ray_config_info_link() {
	...
	echo -e "$green 正在生成链接.... 稍等片刻即可....$none"
	echo
	create_v2ray_config_text >/tmp/v2ray.txt
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file /tmp/v2ray.txt "https://transfer.sh/${random}_v2ray.txt")
	if [[ $link ]]; then
		echo
		echo "---------- V2Ray 配置信息链接-------------"
		echo
...
get_v2ray_config_qr_link() {
	...
	echo $vmess | tr -d '\n' >/etc/v2ray/vmess.txt
	cat /etc/v2ray/vmess.txt | qrencode -s 50 -o /tmp/v2ray.png
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file /tmp/v2ray.png "https://transfer.sh/${random}_v2ray.png")
	if [[ $link ]]; then
		echo
		echo "---------- V2Ray 二维码链接 -------------"
		echo
...
update_v2ray.sh() {
	if [[ $_test ]]; then
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/test/v2ray/test/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	else
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/test/v2ray/master/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
	...
	if [[ $latest_version == $_version ]]; then
		...
		echo
		cd /etc/v2ray/test/v2ray
		git pull
		cp -f /etc/v2ray/test/v2ray/v2ray.sh $_v2ray_sh
		chmod +x $_v2ray_sh
		echo
...
reinstall)
	...
	if [[ $is_uninstall_v2ray ]]; then
		cd
		cd - >/dev/null 2>&1
		bash <(curl -s -L https://git.io/fhZFh)
	fi
	;;
[aA][Ii] | [Dd])
	change_v2ray_alterId
